...
function hexStringToRGB(hexString: string): typeof solution {
  ...
  colours.forEach((element) => {
    const firstValue: string = decimal[hex.indexOf(element[0])];
    //console.log(firstValue)
    const secondValue: string = decimal[hex.indexOf(element[1])];
    //console.log(secondValue)

    const result: number = Number(firstValue) * 16 + Number(secondValue);
